...
# MOCK_TART_VNC_PRINT, MOCK_VNC_URL, MOCK_VNC_PORT,
# MOCK_SLEEP_DELAY,
...
# ---- darwin --gui=vnc × softnet net-policy ---------------------------------
# The URL every verify step above hangs off is Tart's own stdout, and a softnet
# policy is what this repo actually boots under, so the pair is worth pinning
# together: the policy has to reach `tart run` and the boot still has to finish
# at a verified loopback listener.
MOCK_PLATFORM=darwin MOCK_SLEEP_DELAY=0.01 \
  runup stopped app-a "$NETP" "$EMPTY" "$EMPTY" --gui=vnc app-a
assert_rc       "darwin vnc + softnet policy → proceeds" 0
assert_contains "darwin vnc + softnet policy → run carries the net-policy" "$(cat "$CALLS")" "--net-softnet=@host-only"
