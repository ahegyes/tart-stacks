...
│   ├── smoke.sh                        # Characterization tests for script/smoke (stage ordering, EXIT-trap teardown, SMOKE_KEEP, tart-new failure propagation, and the three guest-agent verdicts running ahead of ssh so a missing agent is not misreported as a bad hostname; mocks via the TART_SMOKE_BIN seam plus PATH shims for ssh and tart — no real VM)
│   ├── base.sh                         # Behavioral tests for 00-base.sh's guest-agent substrate gate: both shipped branches lifted out and run against a mocked systemctl (healthy passes; disabled, not-found, static, enabled-runtime and enabled-but-dead all refuse), plus each branch's wording — one has to point at the base image, the other at the unit's journal
...
│   ├── scripts/
│   │   ├── 00-base.sh                  # First. Asserts the two things inherited from the base rather than built here — the guest's os-release ID, and a tart-guest-agent that is both enabled and running — then system update + core dev pkgs + build toolchain + zellij via distro-lib.sh (root)
