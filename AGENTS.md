...
`make` resolves which template to build from the `OS` token: `PLATFORM` is derived by asking which `shared/*/os` file lists it, then `build`/`rebuild` run `packer build … $(PLATFORM).pkr.hcl`. Today `shared/linux/os` is the only such file, so every supported OS resolves to `linux.pkr.hcl`; `check-os` refuses before that lookup ever runs on an unset, unsupported, or unresolvable token, so a bad selector never reaches Packer with a malformed template name.
