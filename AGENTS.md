...
│   ├── tart-new                        # Creates a project VM by cloning a stack base image — `<name> <stack> <os> [<de>]`, the optional <de> selecting a GUI flavor — with the validation `tart clone` lacks (stack exists, image built, no name collision) + `--cpu`/`--memory`/`--disk-size` pass-through; scrubs the stale host-key pin before cloning
...
├── completions/
│   └── _tart-new                       # Zsh completion for tart-new (stack + OS tokens, resource flags); installed by `make setup`
├── test/
...
│   │   ├── authorized-key-lib.sh       # Anti-lockout gate sourced by every platform's 99-finalize.sh: assert_authorized_key_safe refuses a missing, private, or unparseable SSH key before it is authorized (uploaded to /tmp; not run directly)
...
│   ├── darwin/                         # Darwin platform's token list, package abstraction, and provisioners; no 00-stack.sh, darwin.pkr.hcl, or GUI contract yet
│   │   ├── os                          # Supported operating systems, one per line; consumed by the Makefile (check-os, PLATFORM, bootstrap's BASE_IMAGE) and bin/lib/common.sh's tart_is_base_image. Deliberately no desktops file: the macOS desktop is intrinsic, so the platform has no DE axis
│   │   └── scripts/
