...
├── bin/
│   ├── lib/
│   │   ├── common.sh                   # Sourced leaf helpers, never on PATH. Platform resolution lives here: tart_vm_platform reads `tart get`'s OS field for an existing VM, tart_os_platform resolves a token against shared/*/os before one exists. Both REFUSE (nonzero, silent) rather than defaulting to linux — every call site must honour that
...
│   ├── tart-new                        # Clones a stack base image into a project VM, with the validation `tart clone` lacks: stack exists, image built, no name collision, host-key pin scrubbed. Spans every platform via tart_os_platform; a darwin OS refuses a `<de>` (no DE axis on that platform)
...
├── test/                               # Plain-bash suite (`make test`), one file per subject, no framework. Each file's
│                                       # own header states what it covers and — where it greps the script under test
│                                       # instead of running it — why that subject cannot be executed here. Two rules the
│                                       # suite is held to, because both have been violated and shipped green: every
│                                       # refusal case needs a must-pass control beside it, or a broken harness reads as a
│                                       # pass; and a mock's output format is a factual claim about a real tool, so it is
│                                       # CAPTURED from that tool, never asserted from memory.
...
│   │   ├── runtime/
│   │   │   └── display-mode.swift      # Delivered into a RUNNING guest by bin/tart-up on a window boot — NOT a provisioner, so it sits beside scripts/ rather than in it. Establishes the guest's display mode once from the geometry the VM was cloned with: a fresh macOS clone negotiates 1024x768 under --display-refit and keeps it for life
│   │   └── scripts/
...
- **Host (macOS) and guest (VM) live in the same repo.** Everything under `bin/` and `script/` runs on the host, as do `make`/`packer`; everything under `shared/scripts/`, `shared/linux/scripts/`, `shared/darwin/scripts/`, `shared/files/`, and `stacks/*/scripts/`, `stacks/*/files/` runs inside the build VM. `shared/darwin/runtime/` is the one exception on the guest side: Packer never sees it: `bin/tart-up` delivers it into an already-running guest over `tart exec`, so it ships fixes to clones that already exist.
...
| 2 | `shared/linux/scripts/00-base.sh` | root | Asserts the guest's os-release ID matches the build's `OS` — the image name and manifest are both written from `OS`, so a wrong base would ship mislabeled. Then `assert_release_supported`, which reads the guest's own `SUPPORT_END` and refuses a release past end of life: the target release is a hand-maintained pin, and a stale one is otherwise invisible until a repository is purged mid-build months later. Then asserts `tart-guest-agent.service` is both `enabled` (the literal state, since `is-enabled` also exits 0 for `static`/`enabled-runtime`, neither of which survives a clone's first boot) and running: `tart exec` is a host→guest vsock call served by that agent, everything inside the build rides ssh — so an image whose agent is absent or dead builds clean, then silently leaves clones on the base image's hostname (`tart-up` warns) and hard-fails any GUI activation. The package is this build's own: `install_guest_agent` fetches the pinned `TART_GUEST_AGENT_VERSION` and verifies its sha256 before installing, so this assert covers that install, not the base. Then system update, core dev packages, build toolchain, zellij — all via `family-lib.sh` |
...
| 2 | `shared/darwin/scripts/00-base.sh` | root | Asserts `OS=macos` (the image name and manifest are both written from `OS`), that the release is not older than `MACOS_TARGET_RELEASE`, and that the guest agent's LaunchDaemon + LaunchAgent pair is whole — then updates brew, installs mise if the base lacks it, and installs the core set (zellij, jq, wget, gh) + an optional quality-of-life set (htop, mariadb, shellcheck, ripgrep, fd, fzf, bat, git-delta) |
