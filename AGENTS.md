...
│   ├── smoke.sh                        # Characterization tests for script/smoke (stage ordering, EXIT-trap teardown, SMOKE_KEEP, tart-new failure propagation, the guest-agent probe running ahead of ssh so a missing agent is not misreported as a bad hostname; mocks via the TART_SMOKE_BIN seam — no real VM)
│   ├── base.sh                         # Behavioral tests for 00-base.sh's guest-agent substrate gate: the shipped block lifted out and run against a mocked guest (binary absent, unit disabled, unit missing), plus the refusal's wording — it has to point at the base image, not at this build
...
│   ├── scripts/
│   │   ├── 00-base.sh                  # First. Asserts the two things inherited from the base rather than built here — the guest's os-release ID, and an enabled tart-guest-agent — then system update + core dev pkgs + build toolchain + zellij via distro-lib.sh (root)
...
|---|---|---|---|
| 1 | `shared/scripts/00-base.sh` | root | First (`00-` sentinel). Asserts the guest's os-release ID matches the build's `DISTRO` before anything else — the image name and manifest are both written from `DISTRO`, so a wrong base would ship mislabeled. Then asserts an enabled `tart-guest-agent.service`: `tart exec` is a host→guest vsock call served by that agent, nothing here installs it, and every check downstream rides ssh — so a base without it builds clean and fails only once `tart-up` sets a hostname or activates a desktop. Then system update, core dev packages, build toolchain, zellij — all via `distro-lib.sh` |
...
The guest-agent probe is the only stage that leaves ssh. `tart exec` is a host→guest vsock RPC answered by `tart-guest-agent` **inside** the VM — installing tart on the host cannot supply it, and the agent arrives only because the base image already carried one. `bin/tart-up` routes the guest hostname, the display scale and every GUI activation through that channel, so an agent-less image passes every ssh-based check here and then fails a consumer at first use.
