...
│   ├── tart-down                       # Stops a VM and keeps it stopped: records a deliberate-stop mark (before the stop, so a supervisor's poll cannot race it) that tart-supervise honours and tart-up clears on the next start — supervision cannot infer intent, since `tart run` is disowned and leaves no exit status
...
├── script/
│   ├── setup                           # Host install run by `make setup` (symlinks the bin/ commands, zsh completion, idempotent SSH Include + catch-all check, forwards + mounts scaffold, closing tart-ssh-sync run); --uninstall is the inverse (keeps per-VM config, refuses while supervision agents exist)
...
│   ├── tart-down.sh                    # Characterization tests for tart-down (mark/stop ordering, already-stopped and failed-stop paths, prefix resolution, base-image refusal; mocks tart, sandboxed state dir)
