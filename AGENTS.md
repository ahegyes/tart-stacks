...
│   ├── scripts/
│   │   ├── 00-base.sh                  # First script (the release upgrade ahead of it is an inline block in stack.pkr.hcl, not a file). Asserts the three things it does not build — the guest's os-release ID, that the release is not past its support end, and a tart-guest-agent that is both enabled and running — then system update + core dev pkgs + build toolchain + zellij via distro-lib.sh (root)
