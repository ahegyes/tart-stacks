...
│   ├── scripts/
│   │   ├── 00-release-upgrade.sh       # FIRST. Lifts the guest to the release the image ships as (dnf family only; apt bases are current), then reboots — so it owns a provisioner block of its own with expect_disconnect. Policy lives in distro-lib.sh; this is the hook (root)
│   │   ├── 00-base.sh                  # Runs after the release upgrade. Asserts the three things it does not build — the guest's os-release ID, that the release is not past its support end, and a tart-guest-agent that is both enabled and running — then system update + core dev pkgs + build toolchain + zellij via distro-lib.sh (root)
│   │   ├── 99-finalize.sh              # LAST. Authorize SSH key + sshd drop-in + NOPASSWD sudo + lock admin password; writes the provenance manifest incl. the gui: and support-end: lines (root)
...
│   │   ├── distro-lib.sh               # Package-manager abstraction: pkg_install/pkg_refresh/repo_add_mise/install_zellij etc. for dnf (Fedora) and apt (Debian/Ubuntu) families. Also owns the release policy — FEDORA_TARGET_RELEASE, pkg_release_upgrade (capped at dnf's N→N+2), and assert_release_supported
...
The root `stack.pkr.hcl` (one parameterized template, built with `make build STACK=<name> DISTRO=<distro>` from the repo root) defines the provisioner chain combining shared and stack-specific scripts. `DISTRO` is mandatory — there is no default. The supported distros are listed in `shared/distros`. Three scripts have hard ordering constraints — `shared/scripts/00-release-upgrade.sh` must run first, `shared/scripts/00-base.sh` immediately after it, and `shared/scripts/99-finalize.sh` last, hence the sentinel prefixes. The release upgrade leads because it replaces the guest's release wholesale: running it after `00-base.sh` would download a system update the upgrade then discards. It also reboots, so it must be alone in a provisioner block with `expect_disconnect = true` — nothing placed after it in the same block would run. Stack-specific `00-stack.sh` runs immediately after `shared/00-base.sh` in the same root provisioner block; it sources `shared/scripts/distro-lib.sh` and reads the stack's `packages.<family>` file to install native build deps in a distro-agnostic way.
...
|---|---|---|---|
| 1 | `shared/scripts/00-release-upgrade.sh` | root | **FIRST** (`00-` sentinel), in a block of its own with `expect_disconnect = true`. Lifts a dnf-family guest to `FEDORA_TARGET_RELEASE` and reboots; a no-op on apt, whose bases their publisher keeps current. Ahead of `00-base.sh` because that script's first act is a full system update, and updating a release about to be replaced downloads packages the upgrade immediately discards. Alone in its block because it never returns — `dnf offline reboot` only *schedules* the reboot, so the script blocks until the guest goes down; without that, the next provisioner runs inside the system-update boot and reads the OLD release, and the build ships an image labelled as something it is not. No `pause_before` on what follows: the SSH communicator blocks until the guest is reachable again |
