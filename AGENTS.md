...
│   ├── base-darwin.sh                  # Behavioral tests for shared/darwin/scripts/00-base.sh's guest/release/agent asserts: the shipped region lifted out (landmarks distinct from base-linux.sh's, since darwin's os guard is the region's first line rather than following it) and run against a mocked uname; the healthy substrate passes, an OS-vs-guest mismatch refuses and names why ("mislabel"), and a refused release or an incomplete agent each stop the build on their own, driven through stubbed assert_release_supported / install_guest_agent
...
│   │   └── scripts/
│   │       ├── 00-base.sh              # First provisioner. Asserts the guest is Darwin and OS=macos (the image name and manifest are both written from OS, so a mismatch would ship a mislabeled image), that the release is not older than MACOS_TARGET_RELEASE, and that the guest agent's LaunchDaemon + LaunchAgent pair is whole — all via family-lib.sh — then updates brew, installs mise if the base lacks it, and installs zellij + jq (root)
