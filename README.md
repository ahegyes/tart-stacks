...
`make setup` already added the `Include ~/.ssh/config.d/tart-vms` line to `~/.ssh/config`. That file is generated by `tart-ssh-sync` as a single **`tart-*` wildcard** block: it carries user, identity, host-key handling, a `ProxyCommand` that resolves the VM's current IP at connect time, an interactive-login auto-start hook, and your agent-socket forward — so plain `ssh tart-app-a`, JetBrains Gateway, VS Code Remote-SSH, `rsync`, and `git` all reach a VM by name.

Because the block is a wildcard, **a freshly cloned VM works immediately** — `ssh tart-<newname>` needs no per-VM registration. Run `tart-ssh-sync` by hand only to:

- pick up edits to `~/.config/tart-stacks/forwards` (below);
- change the agent socket forwarded into VMs (`TART_AGENT_SOCKET`).
...
- **Agent forward**: your host SSH agent is forwarded into every VM via OpenSSH `ForwardAgent`, so sshd exports `SSH_AUTH_SOCK` in *every* in-VM session — interactive shells and non-interactive `ssh tart-<name> <cmd>` (git, rsync, provisioning) alike — and `git`/`ssh`/`composer` transparently use the host agent. **By default the generator reads `$SSH_AUTH_SOCK`** — whatever agent your shell is wired to. Override with `TART_AGENT_SOCKET=/path/to/socket` (1Password's `~/.1password/agent.sock`, Secretive's container socket, etc.) when you want a specific agent regardless of shell state. (A legacy fixed-path fallback — `RemoteForward /home/admin/.ssh/forwarded-agent.sock` picked up by the in-VM `~/.zshrc` — still works if you wire it manually.)
