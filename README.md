...
1. `make scaffold STACK=<name>` — stamps `stacks/<name>/` from `templates/stack/`: a generic `00-stack.sh` (reads `packages.<family>` for the build OS), a per-platform `scripts/linux/mise-install.sh` + `scripts/darwin/mise-install.sh` each with a hard-gate smoke test, a `tools` declaration (the canonical tool inventory — `script/smoke` runs its probes, and `make test` holds the installers' gates and `files/mise.toml` to it), `files/mise.toml`, `packages.dnf`, `packages.apt`, `packages.brew`, and a `README.md` whose inventory table is generated from `tools` by `make docs`. One parameterized root `linux.pkr.hcl` covers every linux stack and `darwin.pkr.hcl` covers every darwin stack the same way — there's no per-stack Packer file to write.
2. Edit `files/mise.toml` (tool versions), `tools` (one row per tool), and `scripts/linux/mise-install.sh` + `scripts/darwin/mise-install.sh` (install + one `smoke_gate` group per tool row — keep both in sync unless a runtime needs a platform-specific flag); then `make docs` to regenerate the README table. `make test` fails until the four agree. If the stack needs native build deps (e.g., compile-from-source runtimes), add them to `packages.dnf` (Fedora/dnf names), `packages.apt` (Debian/Ubuntu/apt names), and `packages.brew` (Homebrew formula names) — keep all three files aligned.
...
4. Add a row to the stack table at the top of this README. CI auto-discovers `stacks/*/` and cross-products with every `shared/*/os` — no workflow edit needed.

## Adding an OS

1. Add the OS token (one line) to the owning platform's list — `shared/linux/os`, or `shared/darwin/os` for a macOS release. A token must appear in exactly one; `make check-os` refuses one claimed by two.
...
5. CI picks up the new OS automatically: the matrix is `stacks/*` × every `shared/*/os`, each cell validated against the template its platform directory names — so a darwin token is discovered exactly as a linux one is.
...
- **Build fails within a minute on the `tart-guest-agent.service` assert** → on **linux the build owns that package**: `install_guest_agent` (`shared/linux/scripts/family-lib.sh`) downloads the pinned `TART_GUEST_AGENT_VERSION`, verifies its sha256 against the release checksums, and installs it. So the fault is in that install, its download, or the unit it ships — **not in the base image, and re-pulling a base cannot fix it**. Check the install output above the failure. On **darwin** the relationship is the opposite: nothing installs the agent, the Cirrus base ships the LaunchDaemon/LaunchAgent pair and the build only asserts it is whole, so a failure there does mean the base is wrong — re-pull it with `make bootstrap OS=macos`.
