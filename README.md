...
3. `make build STACK=<name> DISTRO=<distro>` — or `packer validate -var stack=<name> -var distro=<distro> linux.pkr.hcl` for a fast HCL pre-check. `<distro>` must appear in `shared/linux/os`.
