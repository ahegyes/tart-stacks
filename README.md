...
`make setup` already added the `Include ~/.ssh/config.d/tart-vms` line to `~/.ssh/config`. That file is generated by `tart-ssh-sync` from `tart list`; its `tart-<name>` aliases carry user, identity, host-key handling, connection multiplexing, and your agent-socket forward — so plain `ssh tart-app-a`, VS Code Remote-SSH, `rsync`, and `git` all work, not just `tssh`.
