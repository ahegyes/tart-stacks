...
#
# This script atomically establishes the final access model in five steps:
#   0. Clean dnf cache (free up image size before locking state).
#   1. Authorize the user's SSH key (uploaded earlier to /tmp/authorized_key.pub).
#   2. Install NOPASSWD sudoers drop-in for admin.
#   3. Write sshd drop-in disabling password auth.
#   4. Lock admin's password (passwd -l).
#
# Bundling all four here makes the constraint "no provisioner can run between
# disabling password auth and Packer disconnecting" structural rather than
# documented — pkr.hcl ordering cannot violate it. Steps 3 and 4 are
# specifically grouped to close the window between drop-in write and
...
# 0. Clean dnf cache before locking down the image. ~200-300 MB of RPMs +
...
# 1. Authorize the user's SSH key.
...
# 2. NOPASSWD sudoers — validated before landing on disk to avoid lockout
...
# 3. sshd drop-in disabling password auth. Drop-in only takes effect on next
#    sshd start (first boot of any clone) — sshd is intentionally not
#    restarted here, so Packer's existing session continues working through
#    step 4. The 00- prefix wins over cloud-init's 50-cloud-init.conf via
...
# fedora-php-tart hardening — applies on next sshd start.
...
# 4. Lock admin's password. After this, SSH key auth (step 1) is the only
#    way in. Packer disconnects immediately after this script returns.
